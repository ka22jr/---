...
ipcMain.handle('get-dashboard', () => {
  ...
  return { today_sales, month_sales, total_inv, low_stock: low_stock.map(m=>({...m,days_left:daysLeft(m.expiry_date)})), expiry_soon, expired_count: expired.length, recent_sales, top_meds, weekly_chart: weekly, alerts: expiry_soon.length + expired.length };
});
